...
| `inapp-whatsnew-popup-and-about-buildinfo` | skill | The in-app What's New launch pop-up (version-gated sheet with a pure, unit-tested decider) and the Settings/About build-info screen (build-time git-commit injection) — paste-ready SwiftUI/shell; in-app implementation only, release-time gating stays in the sibling skill. |
