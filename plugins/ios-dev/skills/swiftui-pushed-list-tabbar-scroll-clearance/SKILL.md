...
Mined from a hand-rolled custom-tab-bar iOS app: doc comment on the existing
`ScrollView` clearance helper incorrectly claimed `List` was exempt; reverting
the fix reproduced the last-row clip, confirming `List` needed the identical
`rootTabBarScrollClearance()` call, not a List-specific variant.
